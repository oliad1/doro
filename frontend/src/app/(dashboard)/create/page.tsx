...
import {
  ...
  CreateCourseStoreProvider,
} from "@/providers/create-course-store-provider";
...
export default function CreatePage() {
  ...
  const table = (rows: any[], columns: ColumnDef<any, any>[]) =>
    useReactTable({
      data: rows,
      columns: columns,
      initialState: {
  ...
  const { setGeneralInfo } = useCreateCourseStore((state) => state);
...
const CourseType = ({}) => {
  const { courseType, setCourseType } = useCreateCourseStore((state) => state);

  return (
    <div className="lg:mx-[20vw]">
      <h1 className="scroll-m-20 pb-2 text-3xl font-semibold tracking-tight pb-5">
        Let's get started. What type of course is it?
      </h1>
      <RadioGroup
      ...
      >
        <Card
          className="w-full hover:bg-card/90"
...
function useTable(
  ...
  const { setField } = useCreateCourseStore((state) => state);
  ...
  const onSubmit = (data: any) => {
    if (isEditing) {
      setField(
        key,
        rows.map((row) => (row.id == editId ? { ...data, id: editId } : row)),
      );
    } else {
      console.log(rows, id);
...
const Assessments = ({ sensors, table }: AssessmentsProps) => {
  const {
    ...
    conditions,
    conditionGroups,
  } = useCreateCourseStore((state) => state);

  const isGeneral = courseType == "general";
  const assessmentGroupsTable = useTable(
    assessmentGroupsSchema({ isGeneral }),
    {
      name: "Assignments",
      weight: 0,
      count: 1,
  ...
  const assessmentsTable = useTable(
    assessmentsSchema,
    {
      name: "Assignments",
      weight: 0,
  ...
  const conditionGroupsTable = useTable(
    conditionGroupsSchema,
    {
      symbol: "",
    },
  ...
  const conditionsTable = useTable(
    conditionsSchema,
    {
      lower: 0,
      formula: "",
  ...
  return (
    <div className="lg:mx-[20vw] space-y-10">
      <TableSheet
        ...
        description={
          "Assessment groups are logical groups of assessments (i.e. Quizzes, Midterms, Finals). Do not make individual assessments for assessment groups."
        }
        table={table(
          assessmentGroupsTable.rows,
          assessmentGroupsCols({
            onEdit: assessmentGroupsTable.openEdit,
            onDelete: assessmentGroupsTable.onDelete,
            isGeneral,
          }),
        )}
        ...
        dropdownValues={
          isGeneral ? conditionsTable.rows : conditionGroupsTable.rows
        }
      />
      <TableSheet
        ...
        description={
          "Assessments are individual items (i.e. Quiz 1, Final Exam, Tutorial 5)."
        }
        table={table(
          assessmentsTable.rows,
          assessmentsCols({
            onEdit: assessmentsTable.openEdit,
            onDelete: assessmentsTable.onDelete,
            isGeneral,
          }),
        )}
      ...
      {!isGeneral && (
        <>
          <TableSheet
            ...
            description={
              "Condition groups are symbols used in formulas (i.e. E (Exam Grade), P (Project Grade))."
            }
            table={table(
              conditionGroupsTable.rows,
              conditionGroupCols({
                onEdit: conditionGroupsTable.openEdit,
                onDelete: conditionGroupsTable.onDelete,
                isGeneral,
              }),
            )}
          ...
          <TableSheet
            ...
            table={table(
              conditionsTable.rows,
              conditionsCols({
                onEdit: conditionsTable.openEdit,
                onDelete: conditionsTable.onDelete,
                isGeneral,
              }),
            )}
            ...
            dropdownValues={
              isGeneral ? assessmentGroupsTable.rows : conditionGroupsTable.rows
            }
          />
        </>
      )}
...
const Personnel = ({ sensors, table }: PersonnelProps) => {
  const { personnels } = useCreateCourseStore((state) => state);

  const personnelsTable = useTable(
    personnelsSchema,
    {
      name: "",
      email: "",
  ...
  return (
    <div className="lg:mx-[20vw]">
      <TableSheet
        ...
        table={table(
          personnelsTable.rows,
          personnelsCols({
            onEdit: personnelsTable.openEdit,
            onDelete: personnelsTable.onDelete,
          }),
        )}
