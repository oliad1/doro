...
export default function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [averageChartData, setAverageChartData] =
    useState<CourseAverageData[]>();
  ...
  useEffect(() => {
    const fetchCoursesInit = async () => {
      ...
      const stats = getCourseStats(
        ...
        !!data.conditions.length && data.author,
        data.conditions,
      );
      ...
      const chartData = data.assessment_groups.map(
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      if (!courseMetadata || !courseMetadata.assessment_groups || !enrollmentId)
        return;
      ...
      const stats = getCourseStats(
        assessment_groups,
        !!conditions.length && courseMetadata.author,
        conditions,
      );
      const newAverage = stats.newAverage;
      const newCompletion = stats.newCompletion;
          ...
          if (assessment.id === assessment_id) {
            if (upsert) {
              if (isGrade) {
                ...
                if (status) {
                  assessment.assessment_statuses[0] = {
  ...
  return (
    <div className="grid lg:grid-cols-4 grid-cols-1 gap-2 pl-0 pt-4 pb-4 pr-2 sm:pl-2">
      <div className="flex flex-col col-span-2 h-min space-y-2 min-w-0">
        <GradeTable
          ...
          upsertMetadata={(
            ...
            isGrade: boolean,
            status?: string,
          ) => updateMetadata(gradeObj, true, value, isGrade, status)}
          deleteMetadata={(gradeObj: any[], isGrade: boolean, field?: string) =>
            updateMetadata(
              gradeObj,
              false,
              undefined,
          ...
          currFormula={formula}
        />
        <CoursePersonnelCard data={personnelData || []} />
      </div>
      <div className="grid col-span-2 space-y-2 h-min min-w-0">
        <CourseStatCard average={average} completion={completion} />
        <CourseCompletionChart isLoading={isLoading} completion={completion} />
        {!(!!courseMetadata?.conditions && courseMetadata.author) ? (
          ...
          <CourseConditionTable symbolValues={symbolValues} />
        )}
        <CourseAverageChart
          isLoading={isLoading}
