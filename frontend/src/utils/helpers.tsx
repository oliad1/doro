...
export const getCourseStats = (
    ...
    group.assessments.forEach((assessment: any) => {
      ...
      if (gradeObj) {
        if (isFormula) {
          ...
          symbolValues[symbol] +=
            assessment.weight * group.weight * gradeObj.grade;
          sum_weights += 1;
        } else {
  ...
  if (isFormula) {
    ...
    for (const condition of conditions!) {
      ...
      if (
        symbolValues[symbol] >= condition.lower ||
        typeof symbolValues[symbol] == "undefined"
      ) {
        ...
        return {
          newAverage: eval(formula),
          newCompletion: (sum_weights / total_weights) * 100,
          symbolValues: symbolArr,
...
export const getRecentAverageDelta = (
...
export const getAssessmentName = (assessment_group: any, index: number) => {
  ...
  if (assessment_group.name.includes("zes")) {
    //Quizzes
  ...
  data.assessment_groups.forEach((assessment_group: any) => {
    ...
    if (assessment_group.drop) {
      ...
      assessment_group.assessments.forEach((assessment: any) => {
        ...
        assessment.weight =
          assessment_group.weight /
          (assessment_group.count - assessment_group.drop);
  ...
  (data.assessment_groups as any[]).forEach((group) => {
    (group.assessments as any[]).forEach((assessment, i) => {
      if (assessment.dates?.length || assessment.due_date) {
        ...
        const base = assessment.dates?.length
