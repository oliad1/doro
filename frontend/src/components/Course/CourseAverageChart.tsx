...
export default function CourseAverageCard({
  isLoading,
  averageChartData,
  ...
  return (
    <Card className="lg:col-span-2 col-span-1 h-min pb-3">
      <CardHeader>
        ...
        <CardDescription>
          Track your progress with your latest moving average.
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="pb-0">
        {isLoading ? (
          ...
          <ChartContainer config={COURSE_AVERAGE_CHART_CONFIG}>
            <ComposedChart
              accessibilityLayer
            ...
            >
              ...
              <XAxis
                ...
                tickFormatter={() => ""}
              />
              <Bar dataKey="grade" barSize={20} fill="var(--chart-1)" />
              <Line
                dataKey="average"
                type="natural"
                stroke="var(--primary)"
      ...
      <CardFooter className="flex items-start flex-col gap-2 text-sm pb-0">
        <div className="flex gap-2 font-medium leading-none">
          Your average has {positiveDelta ? "increased" : "decreased"} by{" "}
          {Math.abs(averageDelta).toPrecision(3)}%
          {positiveDelta ? (
        ...
        <div className="leading-none text-muted-foreground mb-3">
          {positiveDelta ? POSITIVE_MESSAGE : NEGATIVE_MESSAGE}
        </div>
      </CardFooter>
    </Card>
  );
