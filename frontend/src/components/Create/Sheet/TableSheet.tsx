    ...
    >
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        ...
        <Form {...form}>
          <SheetContent>
            <form
              onSubmit={form.handleSubmit(onSubmitClose)}
                ...
                <div className="grid flex-1 auto-rows-min gap-6 px-4">
                  {table.getHeaderGroups().map((headerGroup) => (
                    <div key={headerGroup.id} className="grid gap-3">
                      {headerGroup.headers
                        .filter(
                          (header) => header.column.columnDef.meta?.inputType,
                        )
                        .map((header) => (
                          <FormField
                            key={header.id}
                            ...
                            name={
                              header.column.columnDef
                                .accessorKey as keyof z.infer<typeof schema>
                            }
                            render={({ field }) => (
                              <FormItem>
                                ...
                                <FormControl>
                                  {renderFormField(
                                    field,
                                    header.column.columnDef.meta?.inputType,
                                    dropdownValues,
                                  )}
                                </FormControl>
                                <FormMessage />
